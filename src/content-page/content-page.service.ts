...
export class ContentPageService {
    ...
    if (fetch_data.length < 1) {
      ...
      const saveData = [
        ...
        },
        {
          ...
          title_slug: 'refund_&_cancellations',
      ...
      let data = await this.model.pages.create(saveData);
  ...
  async update(id: string, updateContentPageDto: UpdateContentPageDto) {
    try {
      ...
      const updateResult = await this.model.pages.updateOne(
        { _id: id },
        { $set: { description: updateContentPageDto.description, updated_at: timestamps } }
      );
  
      return { success: true, message: "Message updated successfully" };
    } catch (error) {
      console.error("Update error:", error.message);
      throw new Error(`Error updating content page: ${error.message}`);
    }
  }
