...
export class ElectronicsController {
    ...
    uploadGroceryCsv(@Request() req, @Body() dto: GroceryCsvDto) {
        return this.GroceryService.uploadItemsCsv(req, dto, RestaurantType.Electronics);
    }
    ...
    findAllGroceryItems(@Query() dto: ListGroceryItemsDto) {
        return this.GroceryService.findAll(Number(dto.page), Number(dto.limit), dto.search, RestaurantType.Electronics);
    }
    ...
    async getListOfGroceryForGuest(@Request() req, @Query() query: GetGroceryListDto) {
        ...
        const limit = Number(query.limit ?? 10);
        const search = query.search;
        const restaurant_id = query.restaurant_id?.trim();
        const category_id = query.category_id?.trim();
    ...
    @ApiBearerAuth('authorization')
    @UseGuards(OptionalAuthGuard)
    findByCategory(@Query() query: FindByCategoryDto, @Req() req: any) {
        return this.GroceryService.findByCategoryName(query, query.page, query.limit, RestaurantType.Electronics, req);
    }
    @Get(':id')
    @Roles(UsersType.Admin)
    @ApiBearerAuth('authorization')
