...
export class FaqService {
  ...
  async create(createFaqDto: CreateFaqDto) {
    try {
      const add = await this.model.faqs.create(createFaqDto);
  ...
  async update(id: string, updateFaqDto: UpdateFaqDto) {
    try {
      const update = await this.model.faqs.updateOne({ _id: id }, updateFaqDto);
  ...
  async remove(id: string) {
    try {
      const remove = await this.model.faqs.deleteOne({ _id: id });
